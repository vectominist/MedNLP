...
class SBertQA(nn.Module):
    ...
    def __init__(self, model_name, hidden_dim):
        ...
        # self.post_encoder = nn.TransformerEncoder(
            # nn.TransformerEncoderLayer(hidden_dim, 8, 1024, dropout=0.1), 2)
    ...
    def forward(self, **inputs):
        ...
        # h_repr = self.attention(stem ,chs ,chs).squeeze(1)
        # chs = chs.squeeze(1)
        # prediction = self.cos(h_repr, chs).reshape(shape['chs'][0], 3)
        # prediction = self.pred_head(h_repr).reshape(shape['chs'][0], 3)
        ...
        if labels is not None:
            ...
            # loss = nn.BCEWithLogitsLoss()(correct_prediction, labels)
